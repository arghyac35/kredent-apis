...
export default class UserSubscriber {
  ...
  public onUserSignIn({ _id, name }: Partial<IUser>) {
    const Logger: Logger = Container.get('logger');

    try {
      const UserModel = Container.get('userModel') as mongoose.Model<IUser & mongoose.Document>;

      UserModel.findByIdAndUpdate({ _id }, { lastLogin: new Date() }, { new: true, timestamps: false }, (err: any, res: IUser) => {
        if (err) {
          Logger.error(`🔥 Error while updating lastLogin on event ${events.user.signIn}: %o`, err);
          return;
        }
        if (res) {
          Logger.silly(`User: ${name}'s last login updated: %o`, res.lastLogin);
        }
      });
    } catch (e) {
